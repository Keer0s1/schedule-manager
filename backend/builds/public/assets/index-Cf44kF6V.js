...
`)}</div>`
  ...
  const s = { 1: 'Понедельник', 2: 'Вторник', 3: 'Среда', 4: 'Четверг', 5: 'Пятница', 6: 'Суббота', 7: 'Воскресенье' }, { lessons: t, startDate: n } = e, r = t[0].weekday; return `
  <div class="${i.card}">
    <div class="${i.header}">
...
`)}
  ...
  `
} const C = (e) => { const s = [e[0]]; for (let t = 0; t < e.length - 1; t += 1) { const n = e[t], r = e[t + 1], o = r.lesson - n.lesson; s.push(e[t]), o > 1 && s.push({ type: 'window', totalTime: S(n.endTime, r.startTime) }) } return s }, P = e => (console.log('lessons: ', e), Array.from(new Set(e.map(t => t.weekday))).sort().reduce((t, n) => { const r = e.filter(o => o.weekday === n).sort((o, a) => o.lesson > a.lesson ? 1 : -1); return { ...t, [n]: r } }, {})), F = '_scheduleDashboard_55lss_1', G = '_scheduleGrid_55lss_8', A = '_scheduleHeader_55lss_18', l = { scheduleDashboard: F, scheduleGrid: G, scheduleHeader: A }; async function R() {
  const e = new URL(window.location.href).pathname.split('/')[2], s = v(); async function t() {
    try { const c = await fetch(`/apiv1/teachers/lessons?teacher=${e}&date=${s}`); if (!c.ok) throw new Error(`HTTP error! status: ${c.status}`); return await c.json() }
    catch (c) { console.error('Fetch error:', c) }
  } const { startDate: n, lessons: r } = await t(), o = P(r), a = Object.keys(o); return console.log('sortedLessons', o), console.log('addWindows', C(o[1])), `
...
`)}
  ...
  `
} const I = '_card_1tun1_1', U = '_header_1tun1_18', W = '_description_1tun1_24', h = { card: I, header: U, description: W }; function $({ name: e, description: s, href: t }) {
  return `
  ...
  `
} const B = '_main_19ba6_1', Y = '_header_19ba6_7', K = '_subheader_19ba6_14', z = '_cardsContainer_19ba6_22', u = { main: B, header: Y, subheader: K, cardsContainer: z }; function J() {
  return `
    <div class="${u.main}">
